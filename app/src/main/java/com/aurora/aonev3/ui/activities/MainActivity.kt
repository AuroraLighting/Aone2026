                ...
                if (isFirmwareChecked == true) {
                    OtaHandler.zigbeeFirmwareBundle.observeForever(Observer { zigbeeFirmwareBundle ->
                        val firmwareBundleIncrement = firmwareBundleIncrement
                            val increment =
                                zigbeeFirmwareBundle.optInt("fw_increment", -1)
            ...
            if (it.isConnected) {
                SyncHandler.syncHandlerCoroutineScope.launch {
                    try {
                        ...
                        if (err is NoConnectionError || it.port == null) {
                            it.isConnected = false
                            val credentials = CloudHandler.getCredentials()
                            if (credentials.first.isEmpty()) {
                                finishAffinity()
                                startActivity(Intent(applicationContext, SplashscreenActivity::class.java))
                            NabtoHandler.openTunnel(it, credentials.first)
        super.onResume()
...
//
//        if (fragment is AcquireGatewayFragment) {
...
//                    it.replace(R.id.frame_layout, GroupsFragment.newInstance())
//                    it.commit()
//            }
//        } else {
//            super.onBackPressed()
